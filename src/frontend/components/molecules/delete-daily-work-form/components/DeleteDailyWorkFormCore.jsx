...
/**
 * DeleteDailyWorkFormCore - Multi-step daily work deletion form
 * 
 * Features:
 * - 3-step deletion process (reason → impact → confirmation)
 * - Construction project-specific validation
 * - Real-time validation and impact assessment
 * - Security confirmation with audit logging
 * - Accessibility compliant (WCAG 2.1 AA)
 */
const DeleteDailyWorkFormCore = ({
  formData,
  validation,
  onFieldChange,
  onStepChange,
  ...
  isSubmitting = false,
  className = '',
  'data-testid': testId = 'delete-daily-work-form-core',
}) => {
  const [showPassword, setShowPassword] = useState(false);
  const [impactExpanded, setImpactExpanded] = useState(false);
  const [securityExpanded, setSecurityExpanded] = useState(true);

  const { steps, deletionReasons, impactCategories, security, ui } = deleteDailyWorkFormConfig;
    ...
    <Card elevation={2} sx={{ mb: 3 }}>
      <CardContent>
        ...
        <FormControl fullWidth sx={{ mb: 3 }}>
          <InputLabel id="deletion-reason-label">Why are you deleting this daily work entry?</InputLabel>
          <Select
        ...
        {formData.reason && (
          <TextField
            ...
            placeholder="Please provide additional context for this deletion..."
            helperText={`${(formData.details || '').length}/1000 characters`}
        ...
        {formData.reason && (() => {
          const allReasons = Object.values(deletionReasons).flat();
          const selectedReason = allReasons.find(r => r.value === formData.reason);
          return selectedReason?.requiresJustification && (
            <Alert severity="info" sx={{ mb: 2 }}>
              This deletion reason requires additional justification for audit purposes.
            </Alert>
          );
        })()}
      </CardContent>
    ...
    <Card elevation={2} sx={{ mb: 3 }}>
      <CardContent>
        ...
        <Alert severity="warning" sx={{ mb: 3 }}>
          Please review the potential impact of deleting this daily work entry before proceeding.
        </Alert>
      ...
      <CardContent>
        ...
        <Alert severity="error" sx={{ mb: 3 }}>
          ...
          <Typography variant="body2">
            You are about to permanently delete this daily work entry. All associated data will be removed.
          </Typography>
        </Alert>
          ...
          <CardContent>
            ...
            <Typography variant="body2" sx={{ mb: 1 }}>
              <strong>Reason:</strong> {(() => {
                const allReasons = Object.values(deletionReasons).flat();
                const selectedReason = allReasons.find(r => r.value === formData.reason);
                return selectedReason?.label || formData.reason;
              })()}
            </Typography>
            {formData.details && (
              <Typography variant="body2" sx={{ mb: 1 }}>
        ...
        >
          ...
          <AccordionDetails>
            ...
            <TextField
              ...
              helperText={validation.errors.confirmation || 'Type "DELETE WORK" exactly to proceed'}
              sx={{ mb: 2 }}
            />
            ...
            <FormControlLabel
              ...
              label="I acknowledge that this action is irreversible and will affect project records"
            />
          </AccordionDetails>
        </Accordion>
      </CardContent>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3 }}>
        ...
        <Box>
          {currentStep < steps.length - 1 ? (
            ...
            >
              {isSubmitting ? 'Deleting...' : 'Delete Work Entry'}
            </button>
          )}
        </Box>
      </Box>
...
DeleteDailyWorkFormCore.propTypes = {
...
export default DeleteDailyWorkFormCore;
